#!/usr/bin/env bash
# Validates release artifacts before anything touches git or GitHub.
# usage: validate-release.sh <Bopop.app> <Bopop.dmg> <appcast.xml>

set -euo pipefail

if [[ $# -ne 3 ]]; then
    echo "usage: $0 <Bopop.app> <Bopop.dmg> <appcast.xml>" >&2
    exit 2
fi
...
for path in "$APP_PATH" "$DMG_PATH" "$APPCAST_PATH"; do
    if [[ ! -e "$path" ]]; then
...
MOUNTED=1
MOUNTED_APP="$MOUNT_DIR/Bopop.app"
if [[ ! -d "$MOUNTED_APP" ]]; then
    echo "error: Bopop.app is missing from the mounted DMG" >&2
    exit 1
fi
